...
func main() {
	...
	flag.BoolVar(&featureSyncConnectionToUser, "guac-feature-sync-connection-to-user",
		...
		"Enable feature to synchronize connections to individual users.")
	...
	flag.BoolVar(&featureSyncConnectionToUserGroup, "guac-feature-sync-connection-to-user-group",
		...
		"Enable feature to synchronize connections to dedicated corresponding user groups.")
	...
	if featureSyncConnectionToUserGroup && len(strings.TrimSpace(userGroupPrefix)) == 0 {
		err := errors.New("'userGroupPrefix' needs to be set if the user group synchronization feature is enabled")
